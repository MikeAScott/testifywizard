...
When you see the BUILD SUCCESSFUL message you can deploy the application on the server. You can deploy it on the server by copying the AtestProject/sources/ear/target/AtestProject-ear-1.0-SNAPSHOT.ear file to the JBOSS_HOME/server/default/deploy folder. Then, launch the run.bat file from JBOSS_HOME/bin/ directory to start the server.
